...
void draw_PFratio_QCDMC(){
  ...
  vector<string> procs = {"vbfhi","vbflo","ggf","vh"};
  vector<int> nptbins = {1,1,6,6};
  vector<int> nmjjbins = {1,1,1,1};

  for(int j=0; j<procs.size(); j++){
    for(int i=0; i<nptbins.at(j); i++){
      for(int k=0; k<nmjjbins.at(j); k++){
	
	TFile* f = new TFile(("bbvcc_full/2022/bb/datacards/testModel_qcdfit_"+procs.at(j)+"_"+year+".root").c_str());
	...
	*/
	...
	string title;
    if( j == 3)
	  title = "VH p_{T} bin "+to_string(i+1);
	else if( j == 2 ) 
	  title = "ggF p_{T} bin "+to_string(i+1);
	else if( j == 1) 
	  title = "VBF m_{jj} bin 0";
	else
	  title = "VH m_{jj} bin 0";
	...
	c1->SaveAs(("bbvcc_full/2022/bb/plots/"+bin+".png").c_str());
	c1->SaveAs(("bbvcc_full/2022/bb/plots/"+bin+".pdf").c_str());
      }
    }
  }
