...
void muoncr(bool pass, bool log=true){
  ...
  // Get the year and prefit/prefit/obs from the running directory                                                                                                      
  ...
  string histdirname = "shapes_prefit/" + name+ "/";
  ...
  string text = "PN B Fail Region";
  if( pass )
    text = "PN B Pass Region";
...
void draw(int index, bool pass, string cat, bool log=true){
  ...
  string year ="";
  string year_string = "62.4/fb, Run 3";
  double rZbb = 1;

  // if(thisdir.find("2022") != std::string::npos){
    year = "2022";
    year_string = "7.98/fb, 2022";
  // }
  // else if(thisdir.find("2022EE") != std::string::npos){
  //   year = "2022EE";
  //   year_string = "26.7/fb, 2022EE";
  // }
  // else if(thisdir.find("2023") != std::string::npos){
  //   year = "2023";
  //   year_string = "18.1/fb, 2023";
  // }
  // else if(thisdir.find("2023BPix") != std::string::npos){
  //   year = "2023BPix";
  //   year_string = "9.69/fb, 2023BPix";
  // }
  ...
  TH1D* data_obs;
  if( cat == "vbfhi" || cat == "vbflo" ){
    thisbin = "mjj"+to_string(index+1);
    thisbin_fit= "ptbin0"+cat;

    if( !pass )
      data_obs = (TH1D*)dataf->Get(("vbf_fail_"+thisbin+"_JetData_nominal").c_str());
    else
      data_obs = (TH1D*)dataf->Get(("vbf_pass_"+thisbin+"_JetData_nominal").c_str());
  }
  else{
    if( !pass )
      data_obs = (TH1D*)dataf->Get((cat+"_fail_"+thisbin+"_JetData_nominal").c_str());
    else
      data_obs = (TH1D*)dataf->Get((cat+"_pass_"+thisbin+"_JetData_nominal").c_str());
  }
  ...
  string filename = "bbvcc_full/2022/bb/datacards/testModel_2022/fitDiagnosticsTest.root";
  ...
  if(!VBF){
    VBF = (TH1D*)data_obs->Clone("VBF_empty");
    VBF->Reset();
  }
  VBF->Scale(7.0);
  ...
  cout<<"----------EEEEE------------------"<<endl;
  /* VH */
  TH1D* VH = (TH1D*)VBF->Clone("VH");
  VH->Reset();
  VH->Add((TH1D*)f->Get((histdirname+"WH").c_str()));
  VH->Add((TH1D*)f->Get((histdirname+"ZH").c_str()));
  // VH->Add((TH1D*)f->Get((histdirname+"ggZH").c_str()));
  VH->Scale(7.0);
  VH->SetLineColor(kBlue+1);
  VH->SetMarkerColor(kBlue+1);
  VH->SetLineStyle(2);
  VH->SetLineWidth(3);
  ...
  if(!Wjets){
    Wjets = (TH1D*)data_obs->Clone("Wjets_empty");
    Wjets->Reset();
  }
  // Wjets->Add((TH1D*)f->Get((histdirname+"EWKW").c_str()));
  Wjets->Scale(7.0);
  ...
  if( cat == "ggf" )
    ...
    text2 += "VH category p_{T} bin "+to_string(index+1);
  l4.DrawLatex(0.2,.72,text2.c_str());
  ...
  ggF_sub->Reset();
  TH1D* VH_sub = (TH1D*)VH->Clone("VH_sub");
  VH_sub->Reset();
  ...
  for(int i=1; i<TotalBkg_sub->GetNbinsX()+1; i++){
    ...
    VH_sub->SetBinContent(i,VH->GetBinContent(i)/data_obs->GetBinError(i));
  }
  ...
  //  VH_sub->Draw("histsame");                                                                                                                                              
  data_obs_sub->Draw("pesame");

  if( !log ) name += "_lin";

  c->SaveAs(("bbvcc_full/2022/bb/plots/prefit/"+name+".png").c_str());
  c->SaveAs(("bbvcc_full/2022/bb/plots/prefit/"+name+".pdf").c_str());
